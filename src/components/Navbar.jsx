    ...
    <nav className='bg-[#000] text-white fixed top-0 left-0 right-0 border-b border-white/10'>
        <div className='container mx-auto px-4 py-2 sm:px-6 lg:px-8'>
            <div className='flex items-center justify-between h-16'>
                <div><Link to="/" className='text-xl font-bold'><span className='text-blue-500'>Rupendra</span> CH</Link></div>
                        ...
                        Object.keys(menuItems).map((key) => (
                            <div>
                                <button onClick={() => toggleDropdown(key)} className='hover:text-gray-300 px-3 py-2 rounded-md text-sm font-medium flex items-center'> 
                                    {menuItems[key].title}
                                    <MdKeyboardArrowUp  className={`ml-2 h-5 w-5 transition-transform ${
